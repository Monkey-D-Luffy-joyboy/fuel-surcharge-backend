...
// Returns the inserted events' IDs (in leg order) — Phase 2's confirm/decline
// flow (api/confirm.js) needs these to later tag the events as confirmed or
// delete them on decline.
...
module.exports = async function handler(req, res) {
  ...
  let eventIds;
  try {
    eventIds = await createHoldEvents(plan.calendar, booking, ref, plan.legs);
  } catch (e) {
  ...
  // Phase 2: this token carries everything api/confirm.js needs later — the
  // payment_method_id the customer already entered, the price, and the
  // calendar event IDs to tag/delete — without any database, just this
  // signed, tamper-proof payload. Ryu clicks ONE link in this email; that
  // page (GET) shows a summary and lets him pick Confirm or Decline (POST)
  // from there — see confirm.js's header comment for why this is a two-step
  // GET-then-POST flow rather than the link itself performing the charge
  // (a link-scanning/preview bot fetching the email's links would otherwise
  // risk triggering a real charge or a real decline on its own).
  const token = signToken({
    ...
    eventIds,
    routeLabel: `${booking.route || ''} ${booking.fromCustomAddress || ''} → ${booking.toAddress || ''}`.trim(),
    pickupDateTimeLabel: booking.returnEnabled
      ? `${booking.date} ${booking.time}（帰り: ${booking.returnDate} ${booking.returnTime}）`
  ...
  const emailBody = `
    ...
    <p>
      ...
      <strong>料金：</strong> A$${price}${!booking.paymentMethodId ? ' <span style="color:#E0623A;">(カード情報が保存されていません — 手動でのご案内が必要です)</span>' : ''}
    </p>
    ${fuelInfo ? `
    ...
    <div style="margin:24px 0;">
      <a href="${reviewUrl}" style="display:inline-block;background:#16332F;color:#fff;padding:14px 28px;border-radius:999px;text-decoration:none;font-weight:600;">内容を確認する（決済 / お断り）</a>
    </div>
    <p style="color:#888;font-size:12px;">
      上のボタンから内容をご確認のうえ、決済して確定するか、お断りするかを選択してください。
    </p>
  `;
